	...
	for(;;) {
		...
		else
		{
			if (uxQueueMessagesWaiting(xData)< 2)
			{
				...
				xALDP_Paket.aldp_hdr_byte_1 = PAKET_TYPE_ALDP;
				if(xQAMSettings.bits.bSource_I2C)
				{
					xALDP_Paket.aldp_hdr_byte_2 = ALDP_SRC_I2C; 
				}
				if(xQAMSettings.bits.bSource_UART)
				{
					xALDP_Paket.aldp_hdr_byte_2 = ALDP_SRC_UART; 
				}
				if(xQAMSettings.bits.bSource_Test)
				{
					xALDP_Paket.aldp_hdr_byte_2 = ALDP_SRC_Test; 
				}
